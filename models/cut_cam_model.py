...
class CUTCAMModel(BaseModel):
  """ This class implements CUT and FastCUT model, augmented with CAM attention.
  """
  @staticmethod
  ...
  def __init__(self, opt):
    ...
    if self.isTrain:
      ...
      self.netDLB = networks.define_D(opt.output_nc, opt.ndf, opt.netD, 5, opt.normD, opt.init_type, opt.init_gain, opt.no_antialias, self.gpu_ids, opt)
      ...
      self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD.parameters(), self.netDLB.parameters()), lr=opt.lr, betas=(opt.beta1, opt.beta2))
  ...
  def forward(self, cam=False):
    ...
    self.fake, cam_logits, _ = self.netG(self.real, cam=True)
  ...
  def compute_D_loss(self):
    ...
    pred_fake, fake_GB_cam_logit, _ = self.netD(fake, cam=True)

    self.loss_D_fake = self.criterionGAN(pred_fake, False).mean()

    # Real B
    self.pred_real, real_GB_cam_logit, _ = self.netD(self.real_B, cam=True)
    ...
    self.loss_D = (self.loss_D_fake + self.loss_D_real + D_ad_cam_loss_GB + D_ad_cam_loss_LB + D_ad_loss_LB) * 0.5
    return self.loss_D
  ...
  def compute_G_loss(self):
    ...
    fake_LB_logit, fake_LB_cam_logit, _ = self.netDLB(self.fake_B, cam=True)
    pred_fake, fake_GB_cam_logit, _ = self.netD(fake, cam=True)
    ...
    G_cam_loss_B = self.BCE_loss(
        self.fake_A2B_cam_logit,
        torch.ones_like(self.fake_A2B_cam_logit).to(self.device)
    ) + self.BCE_loss(
        self.fake_B2B_cam_logit,
        torch.zeros_like(self.fake_B2B_cam_logit).to(self.device)
    )
    ...
    self.loss_G = self.loss_G_GAN + loss_NCE_both + G_ad_cam_loss_GB + G_ad_loss_LB + G_ad_cam_loss_LB + self.cam_weight * G_cam_loss_B
    return self.loss_G

  def calculate_NCE_loss(self, src, tgt):
    n_layers = len(self.nce_layers)
    feat_q = self.netG(tgt, self.nce_layers, encode_only=True, cam=False)

    if self.opt.flip_equivariance and self.flipped_for_equivariance:
      feat_q = [torch.flip(fq, [3]) for fq in feat_q]

    feat_k = self.netG(src, self.nce_layers, encode_only=True, cam=False)
